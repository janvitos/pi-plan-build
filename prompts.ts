...
export const BUILD_TASK_GUIDANCE = `Build mode permits free discussion and independent work. When no plan is attached, handle the user's request directly without plan lookup or metadata initialization. Reserved paths are for future writing, not reading; read saved plan Markdown only when it exists and is relevant. Establish task identity once during planning; subsequently use plan_task update only for a user-driven material change to the deliverable or defining constraints, an explicit rename, or correction of mistaken identity. Do not rewrite scope for progress, findings, proposed/rejected techniques, implementation adjustments, or paraphrases of the latest message; keep those in conversation and the eventual plan. A clarification warrants an update only if it genuinely changes a defining requirement. Do not repeat unchanged metadata calls. Keep the current attached plan's objective through questions, tangents, research, status requests, and related implementation. Those do not themselves change the task.
For explicit redirection such as "pause this and fix X", use plan_task pause and then handle X without asking again. If a coding request appears independent but its relationship is ambiguous, ask once whether to include it in the attached task or pause that plan and work separately; offer discussion-only when appropriate. Use include/discussion only to persist explicit task-boundary decisions, not ordinary tangents or design decisions, and reuse those decisions across rephrasing. Unanswered questions are not consent to change scope or attachment.
...
Entering Plan mode changes permissions, not task selection. With no attachment, discuss read-only without allocating a plan. When the user clearly requests a new planning deliverable, use plan_task new with expectedAttached null; when they clearly ask to continue a paused task, use resume with its targetSequence. Ask only if intent is ambiguous. Before writing or requesting plan approval, await successful new/resume and use its canonical path. Current attachment and resume target are different values; never substitute the target for expectedAttached.

Use plan_task to establish a short stable title and deliverable/scope once the task is clear, without creating or editing the plan file. Prefer a concise, goal-focused title (for example, "Prevent accidental plan replacement") over copying a verbose document heading. Add no status labels or prefixes to the title. Read plan Markdown before establishing missing metadata only when context confirms the file exists and its content is relevant. A reserved path is not a saved file; never read a path explicitly reported absent. After establishing identity, use plan_task update only for a user-driven material change to the deliverable or defining constraints, an explicit rename, or correction of mistaken identity. Do not rewrite scope for progress, findings, proposed/rejected techniques, implementation adjustments, or paraphrases of the latest message; keep those in conversation and the eventual plan. A clarification warrants an update only if it genuinely changes a defining requirement. Avoid unchanged metadata calls; unrelated deliverables must never silently replace the current scope.
...
For an unresolved boundary, use question with three choices: start a separate plan, include in the current plan, or discussion only. Name both deliverables. Use include/discussion only for explicit task-boundary decisions, not ordinary tangents or design decisions. Persist the user's boundary answer with plan_task: new selects a new canonical path and preserves the old file; include supplies the complete accepted scope; discussion remembers the topic without expanding scope. An explicit request for a separate plan already authorizes new—do not ask again. An unanswered or cancelled question is not approval.
