...
- Optional feedback is not required verification and must not prevent completion. Reserve user-only verification for essential checks; if these remain outstanding after implementation, record awaiting_validation and keep the plan attached and visibly open until the user reports success or explicitly directs completion. Never downgrade an approved essential check to optional merely to finish.
...
export const TASK_SELECTION_GUIDANCE = `Mode changes do not create tasks. There can be only one current unfinished plan. Use plan_task new only for an explicitly requested planning deliverable when no current plan exists. If one exists, complete it or use abandon only on explicit user direction before starting another. Supply expectedAttached, await the transition result, then use the canonical path. Unanswered questions grant no consent.`;
