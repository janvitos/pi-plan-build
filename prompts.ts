...
export const TASK_SELECTION_GUIDANCE = `Mode changes do not create tasks. There can be only one current unfinished plan. When no current plan exists, use plan_task new for an explicitly requested planning deliverable or a concrete proposed change the user accepts in a planning conversation. Entering Plan, research, informational agreement, or discussion alone does not create a task. Supply expectedAttached: null, a concise title, and scope; await the returned canonical path before writing. If one exists, complete it or use abandon only on explicit user direction before starting another. Supply expectedAttached, await the transition result, then use the canonical path. Unanswered questions grant no consent.`;
