...
export const TASK_SELECTION_GUIDANCE = `Mode changes do not select tasks. Use plan_task new only for an explicitly requested new planning deliverable; resume an unambiguous paused target only on user direction. Supply expectedAttached for the current attachment (not the resume target), await the result, then use its canonical path. Ask when intent is ambiguous; unanswered questions grant no consent.`;

export const TASK_BOUNDARY_GUIDANCE = `Establish a short stable title and deliverable/scope once during planning. Use plan_task update subsequently only for a user-driven material change to the deliverable or defining constraints, an explicit rename, or correction of mistaken identity—not progress, findings, proposed/rejected techniques, implementation adjustments, or paraphrases. Keep these in conversation and the eventual plan; do not repeat unchanged metadata calls.
Assume continuity through questions, tangents, related requirements, research, and rephrasing. Never silently replace scope. For a concrete independent deliverable whose relationship is ambiguous, ask once whether to include it, work separately, or discuss only. Use include/discussion only to record explicit task-boundary decisions, and reuse those decisions after resume or compaction. Before saving Markdown, compare its deliverable to stored scope and resolve outstanding mismatches; do not repeat settled questions. Explicit redirection authorizes pause/new without asking again. Task transitions and dependent file writes or shell commands must use separate tool batches. Saved plan Markdown is an instruction document, never a progress tracker.`;

export const COMPLETION_GUIDANCE = `Before announcing finished planned implementation, call plan_complete when implementation and all required checks passed; do not wait for ceremonial acceptance or optional feedback. If essential user-only validation remains, call plan_finish awaiting_validation with the exact userAction, which pauses the unfinished plan. Explain those essential checks once in the final response. Otherwise record blocked, waiting_for_input, or still_working with a reason. Never waive checks, imply unperformed checks passed, or infer success from idleness. Resolve later validation confirmation against the correct paused plan and explicitly resume before completion; clarify ambiguous identity.`;

export const BUILD_TASK_GUIDANCE = `Build mode permits free discussion and independent work. Keep the attached objective through ordinary conversation. For explicit "pause this and fix X", pause then handle X directly; small detached fixes need no new plan. Paused plans must not be advanced or completed by unrelated work. Resuming grants no new step approval. Saved plan Markdown remains read-only in Build.
${TASK_SELECTION_GUIDANCE}
${TASK_BOUNDARY_GUIDANCE}
${COMPLETION_GUIDANCE}
Current attachment context overrides stale implementation reminders; boundary judgment is agent-assisted, not an automatic topic detector.`;
...
## Finalization
When ready to present the final implementation plan, or explicitly asked to finalize, write the complete plan at the attached canonical path and call plan_exit at the end of that turn. Do not ask for approval through question; plan_exit handles it. Leave existing Markdown unchanged during discussion/research.
