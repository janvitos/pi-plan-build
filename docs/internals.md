...
Messages that ask the user to act use the theme’s `accent` color in bold through one shared helper: the awaiting-validation heading, the step-kickoff guidance, the cancelled-question notice, the `stay`/`step-by-step` mode announcements, and step-control handoffs that await instruction or approval. The awaiting-validation instruction body is separate regular-color Markdown, so it can wrap naturally and present multiple checks as bullets without turning a paragraph into an accent block. Bold keeps the heading and other cues legible when a theme places `accent` near body text. `success` marks state confirmations, `muted` marks bookkeeping and legacy or unknown notices, `warning` is reserved for cautions and degraded state, and `error` for failures. `/plan show` and `/plan history` remain plain reports, and RPC notify types keep their `info`/`warning` levels because no accent level exists there.
