...
The agent uses `plan_task` to establish a concise task identity once and persist explicit user boundary decisions without creating or editing plan Markdown during discussion. Later identity updates are reserved for user-driven material changes to the deliverable or defining constraints, explicit renames, or corrections of mistaken identity. Progress, findings, proposed/rejected techniques, implementation adjustments, and message paraphrases belong in conversation and the eventual plan, not repeated scope updates. A clarification can justify an update if it genuinely changes a defining requirement. `include` and `discussion` record explicit task-boundary decisions, not ordinary tangents or design decisions. It assumes continuity for clarifications, research, and tangents. When a request introduces a concrete independent deliverable, it can ask whether to start a separate plan, include the work in this plan, or keep it discussion-only. Decisions survive resume and are supplied to the agent after compaction; rephrasing a settled topic should not prompt again. A later request to turn discussion into planned work may warrant reconsideration.
...
Multiple paused plans can coexist, but only one is attached. Detaching hides its title/panel and removes its step tools and implementation reminders without losing progress. Resuming preserves any other attached unfinished plan as paused; it does not change mode, execute code, or grant new step approval. The step-runner's existing pause toggle is separate: it pauses execution inside the attached plan rather than detaching it. Completion applies only to the attachment. Entering Plan mode while detached leaves it detached: mode switches and ordinary discussion do not allocate or resume plans. When you clearly request a new planning deliverable or continuation of a paused task, the agent uses new/resume; it asks only if your intent is ambiguous. `/plan new` and `/plan resume` remain manual equivalents. Detached Plan mode permits read-only discussion but cannot write or approve a plan until one is selected.

List results explicitly identify the current attachment, separately from any resume target. Empty reservations are excluded from default task rows; expanded details retain their identity for diagnostics. Stale-attachment errors report the expected and actual attachment without changing state or automatically retargeting the action.
